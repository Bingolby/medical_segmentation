...
class bingo_cc(nn.Module):
    def __init__(self, in_channels, out_channels, num_classes, recurrence = 2, patch_size = 1536, mini_patch_size = 384, output_stride=8, batch = 2):
    ...
    def forward(self, feature):
        ...
        batch = output.shape[0] // my_batchsize
        output_big = output.view(batch, my_batchsize, 512, 48, 48).contiguous()
        output_big = output_big.permute(0, 1, 3, 4, 2).contiguous().view(batch, 4, 4, 48, 48, 512).permute(0, 1, 3, 2, 4, 5).contiguous().view(batch, 192, 192, 512).permute(0, 3, 1, 2).contiguous()

        output_big = self.bingo_process(output_big)

        
        output_big = output_big.contiguous()
        fmap_size = mini_patch_size//self.output_stride
        batchsize = output_big.shape[0]
        fmap_channel = output_big.shape[1]

        output_big_post = torch.Tensor(my_batchsize * batchsize, fmap_channel, fmap_size, fmap_size).to(device)
        for k in range(batchsize):
            for i in range(4):
                for j in range(4):
                    output_big_post[k * my_batchsize + i * 4 + j, :, :, :] = output_big[k, :, i * fmap_size:(i + 1) * fmap_size, j * fmap_size:(j + 1) * fmap_size]
